...
if __name__ == "__main__":
    ...
    parser.add_argument("--modelpath", type=str,
                        default = "/home/lw754/R255/results/singlecell/mmdynamics/RUN_weighted_hiddim[35, 250]_num_epochs250_best_1.0_1.0_0.0/weights/checkpoint_31800.pt",
                        #default="/home/lw754/R255/results/singlecell/mmdynamics/RUN_weighted_hiddim[35, 250]_num_epochs100_best/weights/checkpoint_31800.pt",
    ...
    for donor in donors:
        ...
        if use_wandb:
            wandb.init(project='R255 MMdynamics', name='Explainablity_TCP_ohne_trainiert', config={'model':args.modelpath},
                       resume="allow")
        ...
        real_TCPs = []
        estimated_TCPs = []
        for j, modality in enumerate(['protein', 'rna']):
            print('##################################')
            print(modality)
            with torch.no_grad():
                for i, data in enumerate(test_dataloader):
                    ...
                    real_TCPs.extend(real_TCP[j])
                    estimated_TCPs.extend(estimated_TCP[j])
            ...
            print(f'true: mean:{np.mean(y_true)}, min:{np.min(y_true)}, max:{np.max(y_true)}')
            print(f'true abs: mean:{np.mean(np.abs(y_true))}')
            print(f'pred: mean:{np.mean(y_pred)}, min:{np.min(y_pred)}, max:{np.max(y_pred)}')
